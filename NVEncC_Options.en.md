...
- none
- <int>    ... use an integer
- <float>  ... use a decimal point
- <string> ... use acharacter string

The arguments with [ ] { } brackets are optional.
...
- 2pass-quarter  
  Runs first pass in quarter resolution, which results in larger motion vectors being caught and fed as hints to second pass.
...
Apply filters to audio track. Filters could be seleted from [link](https://ffmpeg.org/ffmpeg-filters.html#Audio-Filters).
...
- **parameters**
  ...
  - hdr2sdr=&lt;string&gt;  
    ...
    - hable  
      Trys to preserve both bright and dark details, but with rather dark result.
      You may specify additional params (a,b,c,d,e,f) for the hable tone-mapping function below.  
...
- **parameters**
  ...
  - method_switch=&lt;int&gt;  (0 - 256)  
    threshold to switch field shift algorithm. 
  ...
  - level=&lt;int&gt;  (0 - 4)  
    ...
    | level | process | target | description |
    |:---|:---|:---|:---|
...
- **parameters**
  - cycle=&lt;int&gt;  (default: 5)  
    num of frame to select frame(s) to be dropped.
...
Drop consecutive duplicate frame(s) and create a VFR video, which might improve effective encoding performance, and improve compression efficiency.
...
- **Parameters**
  ...
  - ythresh=&lt;float&gt;  (default=3, 0-255)  
    Spatial luma threshold to take care of edges. Larger threshold will result stronger denoising, but blurring might occur around edges.
  
  - cthresh=&lt;float&gt;  (default=4, 0-255)  
    Spatial chroma threshold. Larger threshold will result stronger denoising, but blurring might occur around edges.
...
This will reduce encoder artifacts, while preserving the details of original video,
...
- **Parameters**
  - radius=&lt;int&gt;  (default=3, 1-5)   
    radius of filter. Larger value will result stronger denoising, but will require more calculation.
  
  - strength=&lt;float&gt;  (default=0.08, 0.0 - 1.0)   
    Strength of the filter. Larger value will result stronger denoising.
...
- **Parameters**
  ...
  - charcode=&lt;string&gt;  
    Specify subtitle character code to burn in, for text type sub.
  ...
  - scale=&lt;float&gt; (default=0.0 (auto))  
    scaling multiplier for bitmap fonts.  
  ...
  - ts_offset=&lt;float&gt; (default=0.0)  
    add offset in seconds to the subtitle timestamps (for debug purpose).  
...
- Examples
  ...
  Example3: burn in ASS subtitle from file which character code is Shift-JIS
...
- **Parameters**
  - threshold=&lt;float&gt;  (default=128.0, 0 - 255)  
    Threshold used when detecting edges. Raising this value will result stronger sharpening.
...
- **parameters**
  ...
  - spin
    Always keep the CPU monitoring the GPU task to finish. The latency of synchronization will be minimum, but will always utilize 100% of one logical CPU core.
...
- Examples
  ...
  Example: Set process affinity to direct CCX on Ryzen CPUs
...
### --option-file &lt;string&gt;
File which contains a list of options to be used.
